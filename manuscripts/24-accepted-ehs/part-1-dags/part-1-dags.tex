...
(\citeproc{ref-bulbulia_2024_experiments}{Bulbulia 2024d};
...
required to estimate the average treatment effect may be preferred. In
what follows, we will use the term ATE as a placeholder to mean the
average treatment effect, or equivalently the `marginal effect', for a
target population on a pre-specified scale of causal contrast.
...
supplementary materials \textbf{S3} for further discussion on how
...
our symbols. (Supplementary materials \textbf{S1} provides a glossary
for commonly used causal inference terms).
...
(\citeproc{ref-hernan2024WHATIF}{Hernan and Robins 2024}). Faithfulness
...
variables, implying potential dependence (\citeproc{ref-pearl1995}{Pearl
1995}, \citeproc{ref-pearl2009a}{2009}). (Note that `d' stands for
`directional'.)
...
\item
  ...
  blocks the path from \(A\) to \(C\) (\citeproc{ref-hernan2023}{Hernan
  and Robins 2023 p. 78}; \citeproc{ref-pearl2009a}{Pearl 2009}).
\end{enumerate}
...
We say that conditional exchangeability is satisfied if the potential
outcomes are independent of the treatment assignment conditional on the
measured covariates:
...
(\citeproc{ref-pearl2009a}{Pearl 2009};
\citeproc{ref-pearl2012causal}{\textbf{pearl2012causal?}}). Pearl's
...
distributions. Again, nodes and edges in a causal directed acyclic graph
correspond to non-parametric structural equations: these equations
represent causal mechanisms \emph{without} making specific assumptions
about the functional form of relationships. This is a key distinction
from statistical structural equation models (SEMs) commonly used in
social sciences, which rely on causal assumptions but which do not
often, in current practice, evaluate whether causality is identified
(\citeproc{ref-bulbulia2024swigstime}{Bulbulia 2024c}).
...
sciences (see Bulbulia (\citeproc{ref-bulbulia2024swigstime}{2024c}) for
