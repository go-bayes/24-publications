...
\hypersetup{
  pdftitle={Causal Effects of Religious Service Attendance: Evidence Using Novel Measures From A Large National Longitudinal Panel},
...
Novel Measures From A Large National Longitudinal Panel}
...
and volunteerism. Studies 2 and 3 assess the effect of religious service
on the relative risk of receiving aid and financial support from others
during the past week, employing measures designed to minimise
...
causal effects for religion, as for other social behaviours, presents
significant challenges. Most of religion eludes randomised experiments.
Additionally, valid causal conclusions from observational data require
both high-resolution time-series data and robust methods for causal
inference. Few studies combine these characteristics (refer to Kelly
\emph{et al.} (\citeproc{ref-kelly2024religiosity}{2024}) for a recent
research summary and Chen \emph{et al.}
(\citeproc{ref-chen2020religious}{2020}) for a well-designed
...
We defining causal effects as quantitative contrasts between potential
outcomes under intervention on religious service attendance, the
``treatment,'' within a specified population (see Rubin
(\citeproc{ref-rubin2005}{2005}); Neyman
(\citeproc{ref-neyman1923}{1923}), Robins
...
Consider, our approach does not focus on testing predefined hypotheses;
instead, we aim to compute causal effects with high accuracy by
combining appropriate time-series data and robust methods for causal
...
responses exceeding eight were capped at eight (see \emph{Appendix B})
...
We define three targeted causal contrasts (\emph{causal estimands}) in
the form of modified treatment policies
(\citeproc{ref-diaz2021nonparametric}{Dı́az \emph{et al.} 2021};
\citeproc{ref-haneuse2013estimation}{Haneuse and Rotnitzky 2013};
\citeproc{ref-diaz2023lmtp}{Iván Díaz and Schenck 2023}). Let \(A_t\)
denote the treatment, the monthly frequency of religious service. There
are three time points, \(t\in{0,1,2}\), where \(t=0\) denotes the
baseline wave, \(t=1\), the treatment wave, and \(t=2\) the end of
study. \(\mathbf{d}(\cdot)\) denotes a modified treatment policy
...
case symbool \(a_1\). In this study the functions defined by
...
\citeproc{ref-haneuse2013estimation}{Haneuse and Rotnitzky 2013};
\citeproc{ref-diaz2023lmtp}{Iván Díaz and Schenck 2023}). Let \(A_t\)
represent the treatment variable, the monthly frequency of religious
service. In this study there are three distinct measurement intervals,
...
\(f_\mathbf{d}\). When a treatment level is prescribed by the policy,
perhaps contrary to the observed treatment level, we denote this fixed
level by the lowercase symbol \(a_1\). In this study, the functions
defined by \(f_\mathbf{d}\) are interventions that set \(A_1\) to
\(a_1\).
...
\tightlist
\item
  \textbf{Status Quo (Control) Treatment}: Apply no treatment. Each
  expected mean outcome is calculated using the natural (observed) value
  of religious service attendance for each individual.
\end{enumerate}
...
\item
  ...
  of these assumptions in the context of NZAVS studies, see Bulbulia
  \emph{et al.} (\citeproc{ref-bulbulia2023a}{2023}).
\end{enumerate}
...
all functions estimated in this study. Note, the natural value of the
treatment \(A\) is obtained from its observed instances and baseline
...
Buuren 2018}). This method allowed us to reconstruct incomplete datasets
by estimating a plausible value for missing observation. Because only
one data set could be passed to the \texttt{lmtp}, we employed a single
imputation. About 2\% of covariate values were missing at baseline.
Eligibility requires fully observed baseline treatment measures as well
as treatment wave treatment measures.

\textbf{Outcome missingness}: For data missing due to loss-to-follow-up
or sample attrition following the exposure, we applied
non-parametrically estimated censoring weights using the \texttt{lmtp}
package in R (\citeproc{ref-williams2021}{Williams and Díaz 2021}). This
approach adjusts the analyses to account for the non-random nature of
missing outcome data, helping to prevent confounding and selection bias
in the estimated one-year effect of religious service attendance on
prosocial outcomes.
...
complex, high-dimensional covariate spaces \emph{efficiently} without
imposing restrictive model assumptions
...
either the outcome or the treatment model.
...
\citeproc{ref-Ranger2017}{Wright and Ziegler 2017}). Graphs, tables and
output reports were created using the \texttt{margot} package
...
To illustrate the magnitude of the shift interventions being contrasted,
we provide histograms in Figure~\ref{fig-hist}, which display the
distribution of treatment levels during the treatment wave.
Figure~\ref{fig-hist} A: The intervention for regular religious service,
represented in these histograms, affects a larger portion of the sample
compared to the zero religious service intervention.
Figure~\ref{fig-hist} B: presents the intervention for zero religious
service. It affects a smaller portion of the sample compared to the
regular religious service intervention. The comparative analysis of the
`Regular' versus `Zero' interventions addresses the scientifically
intriguing question: what is the effect difference in a scenario where
religious service is universally attended versus completely absent? The
intervention that increases attendance to regular service levels from
the status quo allows us to consider the potential costs and benefits of
widespread religious practice across society. The intervention that
eliminates religious service allows us to consider the potential costs
and benefits of the widespread loss of religious practice across
society.
...
reveals they are not deterministic. States 1, 2, 3, and 5 indicate
relatively frequent movement in and out of these states, suggesting
lower stability.
...
estimates for the causal effects for the three causal contrasts of
interest on reported charitable behaviours at the end the study. The
...
For `friends give time', the effect estimate on the risk ratio scale
...
For `friends give time', the effect estimate on the risk ratio scale is
...
friends give money & 0.92 & 0.81 & 1.04 & 1.41 & 1 \\
...
sample weights) for each analysis, obviously omitting the baseline
outcome.
...
\textbf{Cross-Sectional Community Time Received Result}: the
...
\textbf{Cross-Sectional Community Money Received Result}: similarly, the
exponentiated change for money received yeilds an approximate risk ratio
of b = 1.18; (95\% CI 1.08, 1.27). The monthly risk ratio, after
adjustment, is 1.996. This risk ratio is 1.45 percent greater than the
causal estimate, again revealing the bias of cross-sectional
regressions.
...
of socieity-wide charitable support. At the same time, even if New
Zealand society were to experience the total loss of religious service
attendance, charitable given would not much differ from its present
shape.
...
\item
  ...
  efficiency, and our sensitivity analyses describe the potential
  effects of unmeasured confounding. However, the presence and influence
  of such confounders remain uncertain.
\item
  \textbf{Convergence}: although model misspecification is less likely
  given our use of non-parametric estimation, convergence assumptions
  from non-parametric estimators may affect precision and accuracy
  (\citeproc{ref-van2014discussion}{Laan \emph{et al.} 2014}).
\item
  ...
  exist or by attenuating true effects. Although our methodological
...
\item
  ...
  experience the strongest responses remains an task for future
  research. Such investigations are crucial for making informed policy
  decisions and tailoring advice.
\item
  \textbf{Transportability Unclear}: our findings are specific to the
  New Zealand population, transportability of these results is a matter
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Diverse Causal Questions Yield Diverse Answers}: the contrast
  between `regular' versus `zero' religious service attendance
  demonstrates stronger effects than the `regular' versus `status quo'
  comparison, aligning with theoretical expectations. This underscores
  the importance of precisely framing causal questions based on
  scientific and policy motivations. \textbf{We recommend that
  researchers explicitly state, estimate, and discuss distinct causal
  contrasts tailored to specific scientific and policy interests.}
\item
  \textbf{Cross-sectional Regression Bias}: Our analysis confirms that
  cross-sectional regressions often overstate effects, necessitating
  caution in their interpretative use without robust causal frameworks.
  Observational studies in the study of religious lack robust causal
  methodologies. \textbf{We advise caution when interpreting results
  from cross-sectional data or from longitudinal data with misspecified
  models, as they can either inflate or diminish true effects.} (see
  Westreich and Greenland (\citeproc{ref-westreich2013}{2013})).
\item
  \textbf{Novel Measures of Prosociality: Help Received}: We confirm the
  causal effects of religious service attendance on charitable donations
  and volunteering using self-reported dependency. This finding aligns
  with evidence that New Zealand's religious institutions function as
  productive charities \footnote{\url{https://www.charities.govt.nz/charities-in-new-zealand/the-charities-register/}},
  see also Brooks (\citeproc{ref-brooks2004faith}{2004}); Woodyard and
  Grable (\citeproc{ref-woodyard2014doing}{2014}); Monsma
  (\citeproc{ref-monsma2007religion}{2007}). Orthogonal evidence
  indicates that religious institutions, owing to their low
  administrative costs and reliance on volunteer support, operate
  efficiently as charities (\citeproc{ref-bekkers2011literature}{Bekkers
  and Wiepking 2011}; \citeproc{ref-khanna1995charity}{Khanna \emph{et
  al.} 1995}). Where self-report bias remains a concern: \textbf{We
  recommend employing indicators of help received from others in the
  community.}
\end{enumerate}
...
\setcounter{enumi}{3}
\item
  \textbf{Signals of Family Response to Religious Service Attendance}:
  there are indications of religious service's effects on family
  contributions of time and money, which persist even after adjusting
  for family size. This finding contributes to growing interest in
  models that propose religious institutions function in part to support
  biological fitness (\citeproc{ref-shaver2020church}{Shaver \emph{et
  al.} 2020}). We recommend future into on the role of religious service
  attendance on within-family prosociality.
\item
  \textbf{Distinct Effects of Losing Versus Gaining Religious
  Engagement}: the social consequences of gains and losses of religious
  service attendance differ. \textbf{We recommend that investigators
  distinguish models of loss from models of gain, where possible and
  scientifically or practically relevant, report multiple results} (see
  Van Tongeren \emph{et al.} (\citeproc{ref-vantongeren2020}{2020})).
\item
  \textbf{Social Benefits of Religious Participation Are Considerable}:
  the economic consequences of increased religious participation reveal
  a substantial ``cash value'' in a hypothetical condition where
  religious service is prevalent. This finding may inform conversations
  about the benefits of religion among its discontents.
\item
  \textbf{Limited Social Effects from Declining Religion}:the social
  consequences of living in a society where regular religious service is
  absent do not markedly differ from those of the current society, at
  least in terms of financial effects and population-wide volunteering.
  This finding may inform conversations about religion among its
  enthusiasts who worry about its decline.
\item
  \textbf{Assessing Broader Social Consequences of Religion}: our study
  investigates the social effects of religious service on charitable
  donations and volunteering in New Zealand. Instead of engaging in
  broad debates about the overall goodness of religion, we advocate for
  investigating precise, empirically tractable questions. This approach
  we have taken here, and advocate, balances caution about the
  uncertainties inherent in observational data with openness to the
  signals that extensive time-series data may afford.
\end{enumerate}
...
markov properties for acyclic directed mixed graphs. \emph{The Annals of
Statistics}, \textbf{51}(1), 334--361.
...
bellevue, washington}, Citeseer. Retrieved from
