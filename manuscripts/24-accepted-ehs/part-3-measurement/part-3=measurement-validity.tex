...
comparative questions enables us to clarify whether comparative research
is possible, and also whether it is desirable. Not all questions are
...
Because treatment groups are exchangeable owing to randomisation, it
...
early that in causal inference we must specify:
...
\item
  A scale of contrast.
\item
  A target population for whom a causal effect estimate is meant to
  generalise.
\end{enumerate}
...
\item
  ...
  adjustment methods (refer to supplementary materials \textbf{S2}).
\item
...
indicating a relationship of effect modification in settings where (1)
...
outcome. For example, (\citeproc{ref-robins2004effects}{Robins \emph{et
al.} 2004 p. 2216}) discusses how in non-experimental settings,
mismeasured confounders can introduce bias even when the measurement
errors of the treatment and outcome are uncorrelated and undirected and
there is no treatment effect. This is because mismeasured confounders
will not control for confounding bias. We present an illustration of
this bias in Table~\ref{tbl-terminologyselectionrestrictionbaseline}
...
explanation for why effects differ when the distribution of effect
...
and directed measurement error, and considers how bias correction may be
...
analytic decisions are ambiguous.
