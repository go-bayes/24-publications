...
[Part 3](#id-sec-3) considers biases that arise at the start of a study when there is a mismatch between the sample population and the target population. When there is restriction of the target population in the sample population at baseline, this may bias results. I focus on structural threats to inference when the sample population is (1) too restrictive (for example, too WEIRD) and (2) insufficiently restrictive (WEIRD sampling leads to bias). Here, we discover that population-restriction biases are formally equivalent to measurement error bias.

[Part 4](#id-sec-4) uses Single World Intervention Graphs (SWIGs) to clarify ambiguities that arise when using causal directed acyclic graphs to address population-restriction and measurement-error biases that do not arise from confounding biases. 
