...
A central question in the scientific study of religion is whether religion causes cooperation. However, to quantitatively address this question is challenging. Core features of religion are inaccessible to experiments. On the other hand, in non-experimental observational studies, because people cannot be randomised into religious conditions the task of disentangling correlation from causation is formidable. Moreover, to quantify causality time-series data are required, such that confounders are measured before the intervention of interest (initiating or stopping religious service attendance), and the intervention is observed before the outcomes of interest. Whereas in an experiment, the ordering of baseline, treatment and post-treatment outcome are trivially obtained -- we do not measure outcomes before the experiment -- in observational settings this natural ordering of events cannot be ascertained from the data when only cross-sectional data are available. We are unaware of any longitudinal study that has attempted to emulate an an experiment by correctly ordering covariates, treatment and outcome in time. As such, we have little individual-level evidence with which to evaluate the questions about the causal effects of religious behaviours and actions on cooperative behaviours.   

Here, we leverage comprehensive panel data from `n_participants` who participated in the New Zealand Attitudes and Values Study from years 2018-2021 investigate the causal effects of religious service attendance on prosocial behaviors as measured by: 

1. Self-reports of charitable donations
2. Self-reports of volunteering 
3. Reported voluntary help received from 
  a. Family 
  b. Friends 
  c. People in the community 
4. Reported financial help received from 
  a. Family 
  b. Friends 
  c. People in the community 

Notably, reports of support-received -- either voluntary help or financial help -- avoid the concern that correlated and/or directed measurement error in self-reported church attendance and self-reported prosociality lead to associations in the absence of causation. Furthermore, by controlling for baseline measures of religious service attendance and all pro-sociality outcomes we avoid reverse-causation and impose strong confounding control. 

Finally, we use the outputs of our models to compute the economic or "cash" value of religious service attendance on prosocial behaviors for the New Zealand economy 

<!-- [@bulbulia2023a; @bulbulia2023; @bulbulia2022; @vanderweele2015; @hernan2023]) -->

## Method

### Sample

Data were collected as part of The New Zealand Attitudes and Values Study (NZAVS) is an annual longitudinal national probability panel study of social attitudes, personality, ideology and health outcomes. The NZAVS began in 2009. It includes questionnaire responses from more almost 72,000 New Zealand residents. The study includes researchers from many New Zealand universities, including the University of Auckland, Victoria University of Wellington, the University of Canterbury, the University of Otago, and Waikato University. Because the survey asks the same people to respond each year, it can track subtle change in attitudes and values over time, and is an important resource for researchers both in New Zealand and around the world. The NZAVS is university-based, not- for-profit and independent of political or corporate funding.https://doi.org/10.17605/OSF.IO/75SNB

### Causal Contrast 

Here, we contrast a hypothetical world in which all individuals were attend at least four religious services per month (weekly) and contrast this estimates against outcomes against the natural values of the treatment observed in the data. 



$$ \text{Longitudinal Modified Treatment Policy} = E[Y(a*,a*)|\textcolor{red}{f(A)},L] - E[Y(a^\prime,a^\prime)|\textcolor{red}{f(A)},L] $$

The theoretical estimand of interest is given by a "shift function" or "modified treatment policy" such that:

$$f(A) = \begin{cases} a* = 4 & \text{if } a < 4  \text{ monthly religious service attendance} & \text{else} & a* = a\\ a^\prime = 0 & ~ \end{cases} $$

When computing the effect of the a hypothetical intervention in which all attended weekly religious service, the practical estimand of interest is given by the following modified treatment policy: 


$$ \text{Longitudinal Modified Treatment Policy} = E[Y(a*,a*)|\textcolor{red}{f(A)},L] - E[Y(a,a)|\textcolor{red}{f(A)},L] $$

Where the practical estimand is: 

$$f(A) = \begin{cases} a* = 4 & \text{if } a < 4  \text{ monthly religious service attendance} & \text{else} & a* = a\\ a = a & \text{or the  observed value of}~a  & ~ \end{cases} $$
...
This causal consistency assumption cannot be verified by data.


2.  **Conditional Exchangeability**: we assume that, given specific observed covariates, the treatment assignment is independent of potential outcomes. 

Conditional exchangeability assumption cannot be verified by data, hence we perform sensitivity analyses.

3.  **Positivity**: we assume that every subject has a non-zero probability of receiving the treatment, irrespective of covariates. 

Below, we evaluate positivity by reporting the number of cases in which individuals transitioned from the state of the treatment in the baseline condition to a different state the exposure wave. 
...
The second step of TMLE involves "targeting" these initial estimates by incorporating information about the observed data distribution to improve the accuracy of the causal effect estimate. This is achieved through an iterative updating process, which adjusts the initial estimates towards the true causal effect. This updating process is guided by the efficient influence function, ensuring that the final TMLE estimate is as close as possible to the true causal effect while still being robust to model misspecification in either the outcome model or the treatment model.

A central feature of TMLE is its double-robustness property, meaning that if either the model for the treatment or the outcome is correctly specified, the TMLE estimator will still consistently estimate the causal effect. Additionally, TMLE uses cross-validation to avoid overfitting and ensure that the estimator performs well in finite samples. Each of these steps contributes to a robust methodology for examining the *causal* effects on of interventions on outcomes. The marriage of TMLE and machine learning technologies reduces the dependence on restrictive modelling assumptions and introduces an additional layer of robustness. For further details see [@hoffman2022; @hoffman2023; @díaz2021]. We performed estimation using `lmtp` package [@williams2021].
