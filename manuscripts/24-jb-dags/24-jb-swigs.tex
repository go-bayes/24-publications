...
we focus on the challenges in addressing certain causal questions in
which there is time-varying confounding without treatment confounder
feedback. Table~\ref{tbl-swigtabledeveloped} \(\mathcal{G}_1\) presents
such a structure of time-varying confounding.
...
equation models. Estimation must occur in a stepwise fashion (refer, for
example to Dı́az \emph{et al.}
...
baseline does not change the structure of confounding.
...
\(Y(\mathbf{g})\), but also includes other variables that the treatment
might affect, including the natural value of future treatments. All
backdoor paths must be closed to each member of this set of
counterfactual variables. We call the more stringent assumptions
required for identification in time-varying treatments (or equivalently
longitudinal modified treatment policies
(\citeproc{ref-diaz2023lmtp}{Díaz \emph{et al.} 2023})): \textbf{strong
...
extension of Robins (\citeproc{ref-robins1986}{1986}) dynamic g-formula
for identifying causality under dynamic treatment regimes as follows.
Here I reproduce the key details of their identification algorthim. I
...
Fourth, we task is to ensure conditional independence of the treatment
