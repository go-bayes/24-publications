...
Full variable names, labels, and scales are reported in  [Appendix A](appendix-measures), with label mappings provided to clarify each outcome’s definition.
...
We control for a rich set of confounders measured at baseline, following protocols describe in [@bulbulia2024causal].  We also adjust for physical disability, employment status, partner status, and parenting status, as time-varying confounders, as these may affect both whether one attends religious service and one's health and well-being.
...
### Baseline and Outcome Variables {#appendix-outcomes} 
