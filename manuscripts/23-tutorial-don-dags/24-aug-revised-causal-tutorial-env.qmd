...
abstract: |
  This chapter introduces causal inference within environmental psychology, emphasising the use of causal Directed Acyclic Graphs (causal DAGs) for identifying causal effects. It is structured into four sections: (1) a non-technical primer on causal inference, which explains its relevance to environmental psychology; (2) a tutorial on causal DAGs, which illustrates the how we may approach causal identification problems under assumptions encoded in graphs;  (3) practical illustrations that put causal DAGs to work, and (4) a section offering guidelines for causal inference in environmental psychology. The primary goal of this chapter is to encourage environmental psychologists to create multiple causal diagrams and conduct corresponding analyses for each diagram. This is particularly important when the assumptions necessary for causal inference are unclear or debated. By doing so, researchers can better explore the range of possible causal relationships, which is crucial given that our questions often focus on causal effects, but the data available to answer these questions rarely provide clear-cut conclusions.
authors: 
...
Psychological scientists are familiar with the expression 'correlation does not imply causation.' By ‘correlation,’ we mean a statistical measure that describes the strength and direction of a linear relationship between two variables. Our interests extend beyond simple correlations to more general measures of association. The goal of nearly every statistical method, from simple t-tests to advanced multilevel models, is to estimate magnitudes of association based on data. We obtain statistical estimates of association by measuring features of the world and computing degrees of association.

What, then, is causation?  David Hume offers two definitions of causation in *Enquiries Concerning Human Understanding* (1751): 

Hume's first definition is as follows:
...
This definition is not relevant to our discussion. Hume's second definition is as follows: 

 "Or, in other words, where, **if the first object had not been, the second never would have existed**[@hume1902] (emphasis added).

The key point to notice in Hume's second definition is that it relies on the concept of a counterfactual. To infer a causal effect requires contrasting two states of the world, one in which an event occurs and one in which it does not. It is Hume's counterfactual definition of causation that forms the basis of the concept of causation that has come to dominate the natural and social sciences.

In the natural and human sciences, we say that $A$ causes $Y$ if acting on or manipulating $A$ would lead to a difference in $Y$ compared with a not acting on or manipulating $A$. 

It is evident from the counterfactual definition of causation we require something more than measuring features of the world to infer causality. We must be able to contrast two states of the world only one of which can occur at any given time. Put differently, causal inference requires us to imagine alternative states of the world, only one of which we can observe. 

<!-- 
Suppose, credibly, that smoking causes lung cancer. Suppose further that smoking also reliably causes people to have yellow fingers. We might notice that people with yellow fingers develop lung cancer at a much higher rate than others. However, a public health policy that recommended vigorous hand-cleaning would remain ineffective in the fight against lung cancer. We say that smoking is a common cause of both finger staining and lung cancer. If we were to merely intervene on finger staining this would not affect lung cancer. However, if we were to substantially intervene on smoking, such an intervention would indeed lead to a change in lung cancer rates.  Smoking causes lung cancer; yellow fingers do not.  -->

Although causality cannot be inferred from mere association, it is often the case that associations can be used to infer average causal effects within populations. 

[**Part 1**](#section-part1) introduces the counterfactual framework of causal inference, focussing on the [*three fundamental assumptions**](#sec-three-fundamental-assumptions) necessary to inference average causal effects  -- or 'average treatment effects' -- within specified populations. We build intuition for quantitatively estimating causal effects from data by focussing on randomised controlled experiments, where the assumptions required to infer average treatment effects are satisfied through enforced randomisation and treatment control  [@westreich2012berkson; @hernan2017per; @westreich2015; @robins2008estimation]. 

[**Part 2**](#section-part2) offers an introduction to causal Directed Acyclic Graphs, more commonly known as causal DAGs. We show that within the counterfactual framework of causal inference, causal DAGs offer a powerful and intuitive language for addressing one of the assumptions necessary to identify average treatment effects from data, namely the assumption of conditional exchangeability (also known as the 'no unmeasured confounders' assumption).

[**Part 3**](#section-part3) presents seven worked examples that put causal diagrams to practice. These examples will help applied researchers to understand how to construct causal diagrams to address their specific questions. These examples also reveal the importance of assumptions required to identify causal effects from observational data where randomisation and control is not generally possible. 

[**Part 4**](#section-part4) offers practical reporting guidelines for environmental psychologists and other human scientists interested in inferring causal effects from observational data. A worry in causal inference in observational settings is that the assumptions required to identify causal effects can not generally be verified with data. Moreover, investigators may disagree about their assumptions. Wherever causal assumptions are uncertain or the timing of variables in one's data is unclear, we advocate for reporting multiple causal diagrams and analysis strategies. 

The literature on causal inference and causal diagrams is large, and growing. We conclude by suggesting readings and resources for environmental psychologists and other scientists interested in learning more.
...
Imagine you face a major life decision: you can either start a graduate programme in Environmental Psychology at a University in New Zealand or accept a job offer from a leading renewable energy company. Your choice will shape your future, affecting your lifestyle, income, and social networks. Which option is best for you?

The challenge here is that once you choose one path, you cannot observe how your life would have turned out on the other. If you go to graduate school, you will experience that outcome, but the outcome of taking the job will remain unknown—and vice versa. This is the 'fundamental problem of causal inference': you cannot observe both potential outcomes to quantify the causal effect of your decision [@rubin2005; @holland1986]. Another way of putting this problem is to say that inevitably half the data required to evaluate your life decision will inevitably remain missing. Whichever path you chose, the path not taken remains a "what if?" counterfactual that you can never measure. The fundamental problem of causal inference evident in this imaginary example is realised in every setting in which we might want to quantatively evaluate the difference between two different interventions on the world. Inevitable, the "full data" required to compute this contrast are missing. 

Causal inference has its origins in the early 20th century with Jerzy Neyman’s foundational work on the potential outcomes framework, initially developed to understand the effects of agricultural experiments. Neyman realised that to understand the causal effect of a treatment, it was essential to compare the potential outcomes under different treatment conditions, even though only one of these outcomes could be observed for each unit. This framework laid the groundwork for modern causal inference, later extended by Donald Rubin, who formalised the potential outcomes approach into what is now known as the Neyman-Rubin Causal Model. James Robins further advanced the field by introducing structural nested models and marginal structural models, which are critical for addressing time-varying treatments and confounding. Judea Pearl contributed significantly by developing the framework of causal Directed Acyclic Graphs (DAGs) and formalising the relationship between graphical models and counterfactual reasoning, providing a new set of tools for causal analysis. We will review these tools in Part 2. Together, these and scholars have shaped the development of causal inference, which at its heart is a data driven method for quantitatively contrasting possible states of the world only one of which may be observed. This might sound like a strange idea, but anyone who has encountered a randomised experiment has encountered counterfactual data science.  We next intuitions for causal inference from the familiar example of experiments. 


### Causal inference in Experiments is a Missing Data Problem

Consider an example relevant to environmental psychology. What is the causal effect of access to green spaces on subjective happiness. For simplicity we assume that happiness is measurable, and denote this state using $Y$, where $Y_i$ is the happiness of individual $i$. 

For simplicity, let us assume that "ample access to green space" can be characterised as a binary variable. Define $A = 1$ as "having ample access to green space" and $A = 0$ as "lacking ample access to green space." We assume these conditions are mutually exclusive. This simplification does not limit the generality of our conclusions; the points we make about experiments also apply to continuous treatments. However, because causal effect estimation requires a contrast between predefined treatment conditions, it will be easier to evaluate how randomised controlled experiments estimate causal effects when we restrict attention to a binary treatment. It may seem obvious that to answer a causal question we must first ask it.  However, the practice of defining clear causal questions before attempting an analysis is rarely employed in psychological science outside of experimental work.  

The target of causal inference may be stated as a contrast between potential outcomes under two or more treatment conditions. In this case, the target of inference could be characterised as a contrast between potential happiness states under the treatment "has ample access to green space" and the treatment "lacks ample access to green space". In stating such a question we must also explicitly define the target population for whom we seek to evaluate causal effects. In this case, let us imagine that our target population is all residents of New Zealand in the 2024.
