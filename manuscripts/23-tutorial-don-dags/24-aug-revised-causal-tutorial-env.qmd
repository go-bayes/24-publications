...
How do experiments overcome the fundamental problem of causal inference to obtain causal effect estimates?Experiments do not solve the fundamental problem of causal inference but rather shift causal questions from individual causal effects to average causal effects in a population. Understanding causal inference in experiments illustrates how three core assumptions essential for estimating causal effects are resolved when we shift the question from individuals to populations. These are the (1) Conditional Exchangeability Assumptions (2) The Causal Consistency Assumptions; (3) The Positivity Assumption. We next consider each in turn.

#### Assumption 1: Conditional Exchangeability

We define 

In the first instance, randomisation allows us to identify average treatment effects by ruling out any inherent differences in the groups that would make them to respond differently, on average, to the treatments we seek to contrast. First, we define the expected value of a treatment as the sum of individual potential outcomes for individuals within a specified population:
