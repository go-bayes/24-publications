---
title: "Causal effect of religious service attendance on cooperation: and outcome-wide study"
abstract: |
  This study investigates the causal effects of religious service attendance on cooperation. It combines panel data from a nationally diverse longitudional study of New Zealanders, the New Zealand Attitudes and Values Study (NZAVS, years 2018-2021) with robust methods for causal inference. We investigate three domains of behaviour: (1) self reported charity and volunteering (2) prejudice/acceptance (3) help-recieved from others (a novel measure). Our framework incorporates targeted machine learning models to balance confounders and doubly-robust methods for causal estimation, utilising Targeted Maximum Likelihood Estimation (TMLE). We find that ... .
author: 
...
graph_density_of_exposure_socialising + ggtitle("Socialing")
...
# for graph
graph_table_church_self_reported_behaviour_outcomes <- here_read("group_tab_compare_behaviour_z")
graph_table_church_self_reported_behaviour_outcomes

# for table
table_church_self_reported_behaviour_raw <- here_read("tab_compare_behaviour_raw")
table_church_self_reported_behaviour_raw

# z score table
table_church_self_reported_behaviour_z <- here_read("tab_church_behaviour_z")
table_church_self_reported_behaviour_z

# loss
output_tab_contrast_t2_hours_charity_loss <- here_read('output_tab_contrast_t2_hours_charity_loss')
table_church_self_reported_behaviour_lost_z<- here_read("tab_compare_church_behaviour_lost_z")
graph_table_church_self_reported_behaviour_outcomes_loss_z <- here_read("group_tab_compare_behaviour_z_loss")
tab_compare_behaviour_loss_raw<- here_read("tab_compare_behaviour_loss_raw")


# prejudice

graph_table_church_prejudice <- here_read("group_tab_warm_church")
table_church_prejudice  <- here_read("tab_warm_church")


# HELP RECEIVED
graph_table_help_received <- here_read("group_tab_time_church")
table_church_help_received <- here_read("tab_time_church")

table_church_help_received
graph_table_help_received
...
graph_table_socialising_self_reported_behaviour_z <- here_read("group_tab_charity_time")
graph_table_socialising_self_reported_behaviour_z


table_socialising_self_reported_behaviour <- here_read("tab_behaviour_charity_socialising_raw")

table_socialising_self_reported_behaviour_z <- here_read("tab_charity_time")
table_socialising_self_reported_behaviour_z
table_socialising_self_reported_behaviour


## Prejudice
table_socialising_prejudice_z  <- here_read("tab_warm_socialising")
table_socialising_prejudice_z

graph_table_socialising_prejudice_z  <- here_read("group_tab_warm_socialising")
graph_table_socialising_prejudice_z

# socialising
table_socialising_help_received<- here_read("tab_hours_only_help_received")
graph_table_socialising_help_received <- here_read("group_tab_help_time")
table_socialising_help_received
graph_table_socialising_help_received
...
Third, large scale panel datasets are rare; where they exist, they have yet to be employed to address causality. Thus, opportunities for causal analysis in this area await.
...
The objective of this study is to ascertain whether an intervention engenders a distinct outcome. Given the observational limitations, we cannot directly examine both the factual and counterfactual states. To circumvent this obstacle, we estimate the population average treatment effect (PATE) by contrasting the mean outcomes of the treated and untreated groups.

Three pivotal assumptions are requisite for causal attribution:

1.  **Causal Consistency**: We assume that the observed outcomes align with those that would have materialised under the intervention. Furthermore, we assume non-interference between individual units; each subject's outcome solely hinges on their treatment status. This assumption may be violated if the intervention lacks precise definition.

2.  **Exchangeability**: We postulate that, given specific observed covariates, the treatment assignment is independent of potential outcomes. In psychological research, this resembles random assignment to experimental conditions. In observational settings, we simulate this through controlling for confounding variables that might spuriously link the treatment to the outcome.

3.  **Positivity**: The assumption here is that every subject has a non-zero probability of receiving the treatment, irrespective of covariates. Two scenarios can undermine positivity:
...
These assumptions facilitate a rigorous and meaningful comparison between treated and untreated subjects. They enable us to infer the global impact of the intervention, notwithstanding the inability to observe counterfactual outcomes directly.
...
4.  **Control for previous exposure**: Accounting for prior exposure is imperative for assessing incident exposure as opposed to prevalent exposure. This step enhances confounding control and aids in sidestepping reverse causation and other unmeasured confounders. It ensures that any unmeasured confounder would need to affect both the outcome and the initial exposure, regardless of prior exposure levels, to account for an observed exposure-outcome association [@danaei2012; @hernan2023].
...
To consistently compute the causal relationship between pet ownership and well-being, we employ Targeted Maximum Likelihood Estimation (TMLE) for doubly-robust causal inference. This methodology synthesises treatment and outcome models, thereby fortifying the study against potential model misspecification.
...
Each of these steps contributes to a robust methodology for examining the *causal* relationship between pet ownership and well-being. The marriage of TMLE and machine learning technologies reduces the dependence on restrictive modelling assumptions and introduces an additional layer of robustness. This integration serves as a particularly robust analytical framework for causal inference in examining the relationship between pet ownership and human well-being. For an extended discussion, refer to(For further details see [@hoffman2022; @hoffman2023; @díaz2021]

### Shift estimand for Longitudional Modified Treatment Strategies

Here, we adopt a "shift" estimand, an alternative to conventional causal estimands that often compare full exposure levels against one another[@díaz2021]. This estimand allows for nuanced comparisons of hypothetical interventions, enhancing causal inference. In our case, we evaluate the effect of universal pet ownership—cats in Study 1 and dogs in Study 2—relative to the natural state of owning or not owning a pet. This methodology is implemented using the `lmtp` package in R [@williams2021].

One key advantage of the shift estimand is its capacity for generating actionable policy insights. It allows us to address questions like, "What would happen if everyone owned a pet?" without requiring an extreme counterfactual where everyone is forced to forego pet ownership for comparison.
...
### Discussion Study 1: causal effects of cat ownership on multi-dimensional well-being
...
**Sleep**: the outcome that stood apart was 'Hours sleep'. Here, the PATE causal contrast was -0.057 with a confidence interval ranging from -0.096 to -0.018. The associated E-value of 1.291 suggests that there is reliable evidence for causality. However, the evidence only provisionally supports the notion that gaining a cat may lead to a reduction in sleep hours, and even here the effect size is modest.

**Outliers**: another outcome worth noting is 'PWB your relationships', which had a PATE causal contrast of -0.038. Though the E-value of 1.228 indicates weak evidence for causality, this could be an area for further investigation.

The results thus prompt caution in making substantive claims about the causal impacts of pet ownership, particularly in gaining a cat, on well-being for the New Zealand population. 
...
### Discussion Study 2: causal effects of dog ownership on multi-dimensional well-being
...
Thus, we find limited support for the proposition that acquiring a dog causally affects human well-being, with evidence restricted to exercise levels.
...
table_demographic_vars_cats |>  kbl(format = "markdown")


```

| **Exposure + Demographic Variables** | **N = 5,376**        |
|:-------------------------------------|:---------------------|
| **Only Cat**                         | 2,308 (43%)          |
| **Age**                              | 55 (44, 63)          |
| **Born Nz**                          | 4,294 (80%)          |
| **Children Num**                     | 2.00 (0.00, 3.00)    |
| **Education Level Coarsen**          | NA                   |
| 1                                    | 134 (2.5%)           |
| 2                                    | 1,751 (33%)          |
| 3                                    | 810 (15%)            |
| 4                                    | 1,367 (25%)          |
...
| pacific                              | 144 (2.7%)           |
| asian                                | 227 (4.2%)           |
| **Hlth Disability**                  | 1,577 (29%)          |
| **Household Inc log**                | 11.35 (10.82, 11.81) |
| **Male**                             | 3,309 (62%)          |
| **Nz Dep2018**                       | 4.00 (2.00, 7.00)    |
| **Nzsei13**                          | 56 (44, 69)          |
| **Partner**                          | 3,827 (71%)          |
| **Total Siblings Factor**            | NA                   |
| 0                                    | 269 (5.4%)           |
| 1                                    | 1,232 (25%)          |
| 2                                    | 1,398 (28%)          |
| 3                                    | 926 (19%)            |
...
: Baseline outcome and democgraphic statistics for cat ownership study {#tbl-table_demographic_vars_cats-m}
...
<<<<<<< HEAD
=======
| **Confounding Control Variables** |   **N = 5,376**   |
|:----------------------------------|:-----------------:|
| **Hours Children log**            | 0.00 (0.00, 2.08) |
| **Hours Exercise log**            | 1.61 (1.10, 2.08) |
| **Hours Housework log**           | 2.20 (1.79, 2.71) |
| **Hours Work log**                | 3.43 (0.00, 3.71) |
...
: Additional confounding control variables for cat ownership study {#tbl-table_confounding_control_vars_cats-m}
...
=======
| **Personality Variables** |   **N = 5,376**   |
|:--------------------------|:-----------------:|
| **Agreeableness**         | 5.50 (4.75, 6.00) |
| **Conscientiousness**     | 5.25 (4.50, 6.00) |
| **Extraversion**          | 3.75 (3.00, 4.75) |
| **Neuroticism**           | 3.25 (2.50, 4.00) |
| **Openness**              | 5.00 (4.25, 5.75) |
| **Honesty Humility**      | 5.50 (4.75, 6.50) |

: Personality variables for cat ownership study {#tbl-table_baseline_personality_cats-m}

@tbl-table_confounding_control_vars_cats-m @tbl-table_confounding_control_vars_cats-m and @tbl-table_baseline_personality_cats-m presents sample statistics for baseline responses in Study 1. Baseline variables were included to limit confounding of the the treatment/outcome effect. We additionally included the baseline response for the exposure and for all of the outcomes.
...
| 4                        |    1,292 (25%)    |
| 5                        |     16 (0.3%)     |
| **Alcohol Intensity**    | 2.00 (1.00, 2.00) |
| **Hlth Bmi**             | 26.2 (23.3, 30.0) |
| **Hlth Sleep Hours**     | 7.00 (6.00, 8.00) |
| **Hours Exercise log**   | 1.61 (1.10, 2.08) |
| **Sfhealth your Health** |                   |
| 1                        |     32 (0.6%)     |
| 2                        |     93 (1.8%)     |
| 3                        |    335 (6.4%)     |
| 4                        |     627 (12%)     |
...
: Health variables for the cat ownership study measured at baseline {#tbl-table_health_vars_cats-m}
...
<<<<<<< HEAD
=======
| **Embodied Well-Being Variables** | **N = 5,376** |
|:----------------------------------|:-------------:|
| **Bodysat**                       |               |
| 1                                 |  280 (5.2%)   |
| 2                                 |  517 (9.7%)   |
...
: Embodied well-being variables for the cat ownership study measured at baseline {#tbl-table_embody_vars_cats-m}
...
| **Reflective Well-Being Variables** |   **N = 5,376**   |
|:------------------------------------|:-----------------:|
| **Lifesat**                         | 5.50 (4.50, 6.00) |
| **Pwb Standard Living**             | 8.00 (7.00, 9.00) |
| **Pwb your Future Security**        | 7.00 (5.00, 8.00) |
| **Pwb your Health**                 | 7.00 (6.00, 9.00) |
| **Pwb your Relationships**          | 8.00 (7.00, 9.00) |
| **Self Esteem**                     | 5.67 (4.67, 6.33) |

: Reflective well-being variables for the cat ownership study measured at baseline {#tbl-table_reflective_vars_cats-m}
...
<<<<<<< HEAD
=======
| **Social Well-Being Outcomes** |   **N = 5,376**   |
|:-------------------------------|:-----------------:|
| **Belong**                     | 5.33 (4.67, 6.00) |
...
| 6                              |    1,408 (27%)    |
| 7                              |    497 (9.5%)     |
| **Support**                    | 6.33 (5.67, 7.00) |

: Social well-being variables for the cat ownership study measured at baseline {#tbl-table_social_vars_cats-m}

@tbl-table_health_vars_cats-m @tbl-table_embody_vars_cats-m @tbl-table_reflective_vars_cats-m and @tbl-table_social_vars_cats-m present the outcome variables measured at baseline. Note that all baseline measures of the outcomes were included as controls in each causal effect estimate, along with the baseline exposure.
...
| **Exposure + Demographic Variables** |    **N = 4,132**     |
|:-------------------------------------|:--------------------:|
| **Only Dog**                         |     1,168 (28%)      |
| **Age**                              |     56 (45, 64)      |
| **Born Nz**                          |     3,218 (78%)      |
...
| 3                                    |      116 (2.8%)      |
| 4                                    |      194 (4.7%)      |
| **Hlth Disability**                  |     1,250 (30%)      |
| **Household Inc log**                | 11.39 (10.82, 11.85) |
| **Male**                             |                      |
| -1                                   |     1,694 (41%)      |
| 0                                    |     2,438 (59%)      |
| **Nz Dep2018**                       |  4.00 (2.00, 7.00)   |
...
: Baseline outcome and demographic statistics for dog ownership study {#tbl-table_demographic_vars_dogs-m}
...
<<<<<<< HEAD
=======
| **Confounding Control Variables** |   **N = 4,132**   |
|:----------------------------------|:-----------------:|
| **Hours Children log**            | 0.00 (0.00, 1.79) |
| **Hours Exercise log**            | 1.79 (1.10, 2.08) |
| **Hours Housework log**           | 2.30 (1.79, 2.71) |
| **Hours Work log**                | 3.43 (0.00, 3.71) |
| **Political Conservative**        |                   |
| 1                                 |    195 (4.9%)     |
| 2                                 |     760 (19%)     |
...
: Additional confounding control variables for the dog ownership study {#tbl-table_confounding_control_vars_dogs-m}
...
| **Neuroticism**           | 3.25 (2.50, 4.00) |
| **Openness**              | 5.00 (4.25, 5.75) |
| **Honesty Humility**      | 5.75 (4.75, 6.50) |

: Personality variables for the dog ownership study {#tbl-table_baseline_personality_dogs-m}

@tbl-table_confounding_control_vars_dogs-m @tbl-table_confounding_control_vars_dogs-m and @tbl-table_baseline_personality_dogs-m presents sample statistics for baseline responses in Study 2. Baseline variables were included to limit confounding of the the treatment/outcome effect. We additionally included the baseline response for the exposure and for all of the outcomes.
...
: Health outcomes measured at baseline for the dog ownership study {#tbl-table_health_vars_dogs-m}
...
<<<<<<< HEAD
=======
| **Embodied Well-Being Variables** | **N = 4,132** |
|:----------------------------------|:-------------:|
| **Bodysat**                       |               |
| 1                                 |  218 (5.3%)   |
| 2                                 |  355 (8.6%)   |
...
: Embodied well-being outcomes measured at baseline for the dog ownership study {#tbl-table_embody_vars_dogs-m}
...
| **Reflective Well-Being Variables** |   **N = 4,132**   |
|:------------------------------------|:-----------------:|
| **Lifesat**                         | 5.50 (5.00, 6.00) |
| **Pwb Standard Living**             | 8.00 (7.00, 9.00) |
| **Pwb your Future Security**        | 7.00 (5.00, 8.00) |
| **Pwb your Health**                 | 8.00 (6.00, 9.00) |
| **Pwb your Relationships**          | 9.00 (7.00, 9.00) |
| **Self Esteem**                     | 5.67 (4.67, 6.33) |

: Reflective well-being outcomes measured at baseline for the dog ownership study {#tbl-table_reflective_vars_dogs-m}
...
<<<<<<< HEAD
=======
| **Social Well-Being Outcomes** |   **N = 4,132**   |
|:-------------------------------|:-----------------:|
| **Belong**                     | 5.33 (4.67, 6.00) |
| **Neighbourhood Community**    |                   |
| 1                              |    107 (2.6%)     |
...
: Social well-being outcomes measured at baseline for the dog ownership study {#tbl-table_social_vars_dogs-m}

@tbl-table_health_vars_dogs-m @tbl-table_embody_vars_dogs-m @tbl-table_reflective_vars_dogs-m and @tbl-table_social_vars_dogs-m present the outcome variables measured at baseline. Note that all baseline measures of the outcomes were included as controls in each causal effect estimate, along with the baseline exposure.
...
# ggsave(
#   plot_group_tab_compare_cats_sleep,
#   path = here::here(here::here(push_mods, "plot_group_tab_compare_cats_sleep")),
#   width = 12,
#   height = 8,
#   units = "in",
...
# ggsave(
#   plot_group_compare_dogs_excercise,
#   path = here::here(here::here(push_mods, "plot_group_compare_dogs_excercise")),
#   width = 12,
#   height = 8,
#   units = "in",
#   filename = "plot_group_compare_dogs_excercise.png",
#   device = 'png',
#   limitsize = FALSE,
#   dpi = 600
#  )
