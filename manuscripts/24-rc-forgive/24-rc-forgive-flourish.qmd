---
title: "Causal Effects of Flourishing on Outcomewide Flourishing in New Zealand"
subtitle: "Or whatever title you'd like Richard :)"
abstract: |
   Here, we investigate the effect of a shift intervention on forgiveness on outcomewide flourishing in a longitudinal sample of 33,198 New Zealanders (years 2018 to 2021). We effects.... 
...
tab_health<- here_read("tab_health")
tab_body<- here_read("tab_body")
tab_ego<- here_read("tab_ego")
tab_reflective<- here_read("tab_reflective")
tab_social<- here_read("tab_social")
...
tab_health_1<- here_read("tab_health_1")
tab_body_1<- here_read("tab_body_1")
tab_ego_1<- here_read("tab_ego")
tab_reflective_1<- here_read("tab_reflective_1")
tab_social_1<- here_read("tab_social_1")
...
Does forgiveness causally affect multi-dimensional well-being?  

<!-- We define a causal effect as a quantitative contrast between average outcomes in a population under clearly defined interventions. -->

<!-- To estimate causal effects of forgiveness on multi-dimensional well-being, we use comprehensive panel data from `r n_participants` participants in the New Zealand Attitudes and Values Study from 2018-2021.  A fundamental challenge in observational studies is to ensure *balance* between the variables in interventions to be compared that might affect both treatment and the potential outcomes under treatment [@shiba2021using]. We call the state of imbalance *confounding*, and the strategy for ensuring balance, *confounding control.* In this study, we express the interventions on religious service as "modified treatment policies" [@haneuse2013estimation; @diaz2023lmtp; @díaz2021; @hoffman2023]. We obtain causal inferences by contrasting inferred population averages under different modified treatment policies. -->


Our first causal questions asks: "What would be the average difference across the New Zealand population if everyone below average forgiveness were shifted to average compared with the status quo?" This contrast has a clinical interest.

Our second causal questions asks:  "What would be the average difference across the New Zealand population if everyone in the population were to increase their level of forgiveness by one point on the 1-7 Likert scale, up to a maximum of seven?" This contrast has a general interest.

There are innumerably many causal contrasts we might have considered. Our approach does not focus on testing specific hypotheses; but rather on quantifying magnitudes of difference [@hernan2024stating]. The causal contrasts we define here -- which are standard New Zealand Attitudes and Values Study contrasts for therapeutic and population-wide interventions [@bulbulia2024PRACTICAL] are meant to be conversation starters for audiences with different practical interests. 
...
The New Zealand Attitudes and Values Study measures forgiveness using the following questions: 
...
### Reflective Well-being Outcomes
...
We define three interventions to operationalize our prespecified modified treatment policies (refer to @haneuse2013estimation; @diaz2021nonparametric; @diaz2023lmtp).

Let $A_t$ denote the treatment variable, specifically an individual's level of forgiveness, observed at three time points: $t \in \{0,1,2\}$, where $t=0$ is the baseline wave, $t=1$ the treatment wave, and $t=2$ the end of the study. The function $\mathbf{g}(\cdot)$ denotes a modified treatment policy $f_\mathbf{g}$. Under this policy, we imagine an intervention where each individual receives a treatment level possibly different from their observed level. The lowercase $a_i$ represents the fixed level of treatment according to the modified treatment function $\mathbf{g}$. Since treatment modifications occur at a single interval (wave = baseline + 1), we avoid complexities arising from time-varying treatments (cite robins..).

1. **Therapeutic Intervention**: Elevate forgiveness levels below the population average up to the population average; maintain others unchanged:
   $$
   \mathbf{g}^\lambda(A_i) = \begin{cases} 
   \overline{A} & \text{if } A_i < \overline{A} \\ 
   A_i & \text{otherwise} 
   \end{cases}
   $$
   where $\overline{A} = \frac{\sum_{i=1}^n A_i}{n}$.

2. **General Intervention**: Increase everyone's forgiveness level by one point on the Likert scale, capping at a maximum score of seven:
   $$
...
3. **Status Quo — No Treatment**: Apply no treatment. Expected mean outcomes are computed using each individual's natural (observed) forgiveness level:
   $$
   \mathbf{g}(A_i) = A_i
   $$
...
**Contrast A: Therapeutic Intervention vs. Status Quo**
   $$
   \tau^{\text{therapeutic}} = E[Y(\mathbf{g}^\lambda(A_i)) - Y(\mathbf{g}(A_i))]
   $$

**Contrast B: General vs. Status Quo**
   $$
   \tau^{\text{general}} = E[Y(\mathbf{g}^\phi(A_i)) - Y(\mathbf{g}(A_i))]
   $$

#### Assumptions for Causal Estimation

To consistently estimate a causal effect, three assumptions must be satisfied:

1. **Causal Consistency**: Potential outcomes correspond with observed outcomes under the treatments in the data. We assume that potential outcomes are not influenced by how the treatment was administered, conditional on measured covariates (@vanderweele2009; @vanderweele2013).

2. **Exchangeability**: Given observed covariates, treatment assignment is assumed to be independent of the potential outcomes to be contrasted, eliminating "unmeasured confounding" (@hernan2024WHATIF; @chatton2020).

3. **Positivity**: There must be a non-zero chance of receiving each treatment level within the subgroups defined by covariates necessary for exchangeability (@westreich2010).
...
@tbl-02 presents causal diagram describe the general confounding control of @vanderweele2020. 
...
The second step of TMLE involves "targeting" these initial estimates by incorporating information about the observed data distribution to improve the accuracy of the causal effect estimate [@van2014discussion].

Again, a central feature of TMLE is its double-robustness property. If either the treatment model or the outcome model is correctly specified, the TMLE estimator will consistently estimate the causal effect. Additionally, we used five-fold cross-validation to avoid over-fitting, following the protocols stated in @bulbulia2024PRACTICAL. For further details of the specific targeted learning strategy we favour, see [@hoffman2022; @hoffman2023; @díaz2021]. We perform estimation using the `lmtp` package [@williams2021]. We used the `superlearner` library for semi-parametric estimation with the predefined libraries `SL.ranger`, `SL.glmnet`, and `SL.xgboost` [@polley2023; @xgboost2023; @Ranger2017]. We created graphs, tables and output reports using the `margot` package [@margot2024]. 
...
To illustrate the magnitude of the shift interventions we contrast, we provide histograms in @fig-hist, that display the distribution of treatments during the treatment wave. @fig-hist *A*: clarifies the shift required for the therapeutic intervention. @fig-hist *B*: clarifies the shift required for the 'general' intervention.
...
graph_density_of_exposure_up <- margot::here_read("graph_density_of_exposure_up")
graph_density_of_exposure_up + theme_classic()
```
{{< pagebreak >}}
