...
In Pearl's formalism, we assume that $U_L$, $U_A$, and $U_Y$ are independent exogenous random variables. That is, we assume there are no direct arrows linking $A$ to $Y$ except through the common cause node $L$. Causal diagrams allow us to factorize the joint distribution of $L$, $A$, and $Y$ as a product of conditional probability distributions.
