...
\hypersetup{
  pdftitle={The Transformation of The Obvious: Causal Inference for Interaction, Mediation, and Time-Varying Treatments},
...
to confusion. We use advances in causal inference to explain why the
quantities derived from statistical models routinely employed in the
human sciences are often ambiguous. We emphasise the necessity of (1)
clearly stating a causal question and (2) assessing the identifiability
of the question with available data before statistical analysis. Without
these steps, confusion is inevitable. Properly framing and addressing
causal questions of interaction, mediation, and time-varying treatments
reveals the limitations of popular methods and guides researchers to
clearer understanding of the phenomena that animate our interests.
...
(\citeproc{ref-westreich2013}{Westreich and Greenland 2013}). It is
...
communicating results (\citeproc{ref-hernan2024WHATIF}{Hernan and Robins
2024}; \citeproc{ref-tlverse_handbook}{Laan \emph{et al.} 2023};
...
\subsection{Fundamental Assumptions for Causal
...
these assumptions, the ideal experiment satisfies them. In observational
or `real-world' settings, none of these assumptions are guaranteed. Only
the positivity assumption can be verified by data.

\subsection{Workflow for Inferring Causal Effects from Real-World
...
\item
  ...
  no unmeasured confounding. The goal is to ensure non-random
  `real-world' data can emulate a randomised controlled experiment.
\item
  \textbf{Check if the positivity assumption is satisfied.}
\item
  \textbf{Clearly communicate the reasoning, evidence, and
  decision-making that inform steps 1-6.}
\end{enumerate}
...
graphs can be found in Pearl (\citeproc{ref-pearl2009a}{2009}); Barrett
...
(\citeproc{ref-bulbulia2023}{2023}). For an introduction to Single World
Intervention Graphs refer to Richardson and Robins
(\citeproc{ref-richardson2013}{2013a}).
...
2020}). A time-indexed node, \(X_k\), allows us to index measurements
...
(\citeproc{ref-pearl1995}{Pearl 1995}). This means that, on the basis of
...
acyclic graph Pearl (\citeproc{ref-pearl1995}{1995}). Here, we use the
...
(\citeproc{ref-terminologygeneral}{\textbf{terminologygeneral?}}).
...
Tibshirani \emph{et al.} (\citeproc{ref-grf2024}{2024}); Vansteelandt
and Dukes (\citeproc{ref-vansteelandt2022a}{2022})). Alternatively we
...
Outcomes A = 0: - \(\mu_{00} = 5\) - \(\mu_{01} = 15\)

Outcomes A = 1: - \(\mu_{10} = 10\) - \(\mu_{11} = 20\)
...
(\citeproc{ref-richardson2013}{Richardson and Robins 2013a}). By
contrast causal directed acyclic graphs such as \(\mathcal{G}_1\)
require positing interventional distributions. The formalism that
underpins these intervential distributions is mathmatically equivalent
to that of the potential outcomes framework -- if we assume that the
errors the underlying structural causal models that define the nodes on
which interventions occur are independent
(\citeproc{ref-richardson2013}{Richardson and Robins 2013a}). Not only
do SWIGs allow us to evaluate identification when errors are not
independent, more basically the allow project distinct interventions to
be compared onto our causal diagramme. This is helpful when more than
one point intervention is considered.}
...
Single World Intervention Graphs Table~\ref{tbl-swigtable}
\(\mathcal{G}_3\) and Table~\ref{tbl-swigtable} \(\mathcal{G}_4\)
present separate causal diagrams for each treatment to be contrasted. A
Single World Intervention Template \(\mathcal{G}_2\) acts as a `graph
value function' that produces the individual counterfactual graphs
(\citeproc{ref-richardson2013}{Richardson and Robins 2013a}). In
contrast, causal directed acyclic graphs (DAGs) such as
\(\mathcal{G}_1\) require positing interventional distributions. The
formalism underpinning these interventional distributions is
mathematically equivalent to the potential outcomes framework, assuming
the errors in the underlying structural causal models defining the nodes
on which interventions occur are independent
(\citeproc{ref-richardson2013}{Richardson and Robins 2013a}). However,
...
\citeproc{ref-wheatley1971pivot}{Wheatley 1971}) -- mediation analysis
...
in causal mediation analysis (\citeproc{ref-robins1992}{Robins and
Greenland 1992}). This landmark paper has been to mediation analysis
...
VanderWeele (\citeproc{ref-vanderweele2015}{2015}) given that our
...
government; see: Williams and Díaz (\citeproc{ref-williams2021}{2021});
Díaz \emph{et al.} (\citeproc{ref-duxedaz2021}{2021}); Hoffman \emph{et
al.} (\citeproc{ref-hoffman2022}{2022}); Hoffman \emph{et al.}
...
complicates causal inferences (\citeproc{ref-mcelreath2020}{McElreath
2020 pp. 123--144}). We expand on this example by first clearly stating
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.4778}}
...
(\citeproc{ref-robins1986}{1986}) dynamic g-formula. Essentially, the
algorithm is:
...
al.} 2023}). It is also possible to write stochastic treatment functions
...
(\citeproc{ref-freeman1948ancilla}{Freeman 1948}). Hume provided a
...
existed' (\citeproc{ref-hume1902}{Hume 1902}). However, it was not until
Jerzy Neyman's master's thesis that a quantitative analysis of causality
was formalised (\citeproc{ref-neyman1923}{Splawa-Neyman \emph{et al.}
1990}). Remarkably, Neyman's work went largely unnoticed until the 1970s
...
(\citeproc{ref-pearl1995}{Pearl 1995}). Robins and Richardson extended
...
causal mediation analysis methods (\citeproc{ref-pearl2009a}{Pearl
2009}; \citeproc{ref-robins1992}{Robins and Greenland 1992};
\citeproc{ref-rudolph2024mediation}{Rudolph \emph{et al.} 2024};
\citeproc{ref-vanderweele2015}{VanderWeele 2015};
\citeproc{ref-vanderweele2014a}{VanderWeele and Vansteelandt 2014};
\citeproc{ref-vansteelandt2012}{Vansteelandt \emph{et al.} 2012};
\citeproc{ref-diaz2023}{\textbf{diaz2023?}}), and techniques for
...
(\citeproc{ref-robins1986}{1986}) dynamic g-formula for identifying
