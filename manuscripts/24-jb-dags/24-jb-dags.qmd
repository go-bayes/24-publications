...
abstract: |
  In causal inference, accurately quantifying a causal effect requires contrasting hypothetical counterfactual states simulated from data. This complex task relies on a framework of explicit assumptions and systematic, multi-step workflows that begin by stating a clear causal question and a clear target population for whom inferences are meant to be valid. Causal directed acyclic graphs (DAGs) are tools within this multi-step workflow. We deploy them after we have stated our causal question and stated our target population, to help us understand whether and how the counterfactual contrasts we target can be identified from data, based on assumptions we obtain from previous science (structural assumptions). This guide offers practical advice for constructing causal directed acyclic graphs that are effective and safe. These causal graphs make clear causal inference’s mission-critical demand for obtaining accuracy in the relative timing of events of confounders, treatements, and outcomes, and how, even when timing is ensured, valid causal inferences remain elusive.
